...
class AssetIndexer:
    ...
    def build_index(self) -> dict:
        ...
        # 4. Index core lineage artifacts from graph if available
        ...
        if graph_db.is_file():
            try:
                ...
                c = conn.cursor()
                c.execute("""
                    SELECT DISTINCT target_node FROM jhoc_graph_relation 
                    WHERE target_node LIKE 'file:%' AND relation_type IN ('touches_file', 'observed_in')
                    LIMIT 200
                """)
                for row in c.fetchall():
                    ...
                    rec = AssetRecord(
                        asset_id=f"artifact-{fname}",
                        asset_type=AssetType.SCRIPT if fname.endswith(".py") else AssetType.ARTIFACT,
                        title=fname,
                        path=fpath,
                        intent_affinity=("DETERMINISTIC_ENGINEERING",),
                        triggers=(fname, fpath),
                        content_summary=f"Lineage Graph Asset: {fpath}",
                    )
                    assets.append(rec.to_dict())
                    for bg in set(compute_char_bigrams(fname)):
                        bigram_index.setdefault(bg, []).append(rec.asset_id)
                conn.close()
            except Exception as e:
                print(f"[WARN] Failed to index graph artifacts: {e}")

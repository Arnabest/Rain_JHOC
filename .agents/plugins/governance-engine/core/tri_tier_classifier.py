        ...
        if index_data:
            ...
            if asset_scores:
                ...
                if best_count >= 2:
                    ...
                    ephemeral = list(LessonTemplateRenderer.render_ephemeral_package(
                    ...
                    # Graph-RAG context augmentation for touched artifacts
                    try:
                        graph_db = self.root / "logs" / "jhoc-lineage-graph.sqlite"
                        if graph_db.is_file():
                            from jhoc.graph.sqlite import SQLiteGraphStore
                            from jhoc.graph.retriever import GraphRAGRetriever

                            gstore = SQLiteGraphStore(str(graph_db))
                            gretriever = GraphRAGRetriever(gstore)
                            lineage = gretriever.find_file_lineage(rec.path)
                            gstore.close()

                            if lineage.get("upstream_sessions") or lineage.get("related_errors"):
                                from jhoc.context.sanitizer import DataSanitizer
                                head_line, _ = DataSanitizer.sanitize_text(
                                    f"[GRAPH-RAG RECALL] File: {rec.title} | Sessions: {len(lineage['upstream_sessions'])} | Errors: {len(lineage['related_errors'])}"
                                )
                                ephemeral.append(head_line)
                                for err in lineage.get("related_errors", [])[:2]:
                                    clean_err, _ = DataSanitizer.sanitize_text(f"  [HISTORICAL PRECEDENT ERROR] {err}")
                                    ephemeral.append(clean_err)
                                for sess in lineage.get("upstream_sessions", [])[:2]:
                                    clean_sess, _ = DataSanitizer.sanitize_text(f"  [ORIGIN SESSION] {sess}")
                                    ephemeral.append(clean_sess)
                    except Exception:
                        pass

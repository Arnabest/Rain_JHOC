...
canonical_id: beautiful-article
aliases: ["beautiful-article", "网页长文", "生成文章", "reacticle", "精美文章"]
version: 1.0.0
category: presentation
mutable_by_agent: false
trigger: ["beautiful-article", "网页长文", "生成文章", "reacticle", "精美文章", "文章制作", "长文排版"]
when_to_use: ["把用户提供的素材（URL/PDF/DOCX/MD/文本/截图）编辑设计为可离线打开的单文件 HTML 网页文章", "基于 reacticle 组件协议排版生成高密度、带微交互、精美排版的长文与 PDF", "制作技术研报、深度解释文、教程、事故审阅长文"]
...
# Beautiful Article (reacticle 网页长文生成引擎)

> **Authority**: Governed under **JHOC Agent 宪法体系** ([AGENTS.md`](AGENTS.md)) 与 [`ADR-0009-registry-shelf-quota.md`](docs/adr/ADR-0009-registry-shelf-quota.md)
> **Physical Location**: .agents\skills\beautiful-article\`
> **Design Pattern**: reacticle 语义组件协议 + 逃生舱，单文件离线 HTML 网页文章与 PDF 生成引擎。
...
Phase 2  Editorial Planning 一份 plan.md（Brief / Outline / Theme / Assets 四段）
   ...
   ->
Phase 3  Plan Checkpoint   [GATE] Checkpoint 1 必须停。逐项确认 5 件事：文章类型（含标配保留比例）/ 主题 / 版式 / 配图模式 / 封面
   ->
Phase 4  First Spread      首屏 + 第一节 + 一个代表性视觉块（脚手架在此创建）
         └ First Spread Reviewer SubAgent（写 review/first-spread-review.md）
         └ [GATE] Checkpoint 2 必须停。逐项确认 2 件事：验收结论 / 开发模式 A/B
   ->
...
Phase 7  Repair            最小切片修复，有修复才写 repair-log.md
   ->
Phase 8  Delivery          [GATE] Checkpoint 3 必须停。逐项确认交付决策 → 交付 article.html + 简短编辑说明
```
...
## Phase 3 —— Plan Checkpoint（[GATE] 硬节点 · Checkpoint 1，必须停）
...
## Checkpoint 2 · First Spread（[GATE] 硬节点，必须停）

...
- [FAIL] 不在脚手架强行装任何 PDF 相关 npm 包（保持脚手架轻量）。
- [FAIL] 不在 Checkpoint 3 默认勾选 "导出 PDF"（PDF 不是主交付物）。
- [FAIL] 不替用户判断"要不要 PDF" —— 这是 Checkpoint 3 用户独立选择项。
- [FAIL] 不改 reacticle 来支持 PDF（CSS 注入更轻量、跟 reacticle 版本解耦）。

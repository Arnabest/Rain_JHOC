...
if [[ -d "$TARGET" && -n "$(ls -A "$TARGET" 2>/dev/null || true)" ]]; then
  echo "[FAIL] 目标目录 '$TARGET' 已存在且非空，已中止。" >&2
  exit 1
fi
if ! command -v npm >/dev/null; then
  echo "[FAIL] 需要 npm，但在 PATH 里没找到。" >&2
  exit 1
fi
...
if npx tsc --noEmit; then
  ...
  echo "[WARN] typecheck 有问题（见上），dev / build 仍可能正常 —— 请人工确认。" >&2
fi

cat <<EOF

[OK] 完成。工作区：$TARGET（主题 $THEME，见 .theme；reacticle $INSTALLED_REACTICLE）

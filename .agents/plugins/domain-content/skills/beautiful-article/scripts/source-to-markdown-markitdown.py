...
def load_markitdown():
    if sys.version_info < (3, 10):
        print(
            "[FAIL] MarkItDown requires Python 3.10+; current Python is "
    ...
    try:
        ...
        print("[FAIL] MarkItDown is not installed in this Python environment.", file=sys.stderr)
        print(f"  Install with: {INSTALL_HINT}", file=sys.stderr)
...
def main() -> None:
    ...
    try:
        ...
        print(f"[FAIL] MarkItDown 转换失败：{exc}", file=sys.stderr)
    ...
    if args.output:
        ...
        print(
            f"[OK] MarkItDown 写入 {out}（{len(markdown)} 字符）。"

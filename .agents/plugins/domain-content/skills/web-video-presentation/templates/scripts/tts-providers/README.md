...
tts_check() {
  ...
  command -v jq   >/dev/null || { echo "[FAIL] jq not found"   >&2; return 1; }
  [[ -n "${ELEVENLABS_API_KEY:-}" ]] || { echo "[FAIL] ELEVENLABS_API_KEY not set" >&2; return 1; }
}
...
tts_check() {
  command -v edge-tts >/dev/null || { echo "[FAIL] edge-tts not found" >&2; return 1; }
}
...
tts_check() {
  command -v say     >/dev/null || { echo "[FAIL] 'say' not available (macOS only)" >&2; return 1; }
  command -v ffmpeg  >/dev/null || { echo "[FAIL] ffmpeg not found (brew install ffmpeg)" >&2; return 1; }
}
...
tts_check() {
  command -v curl >/dev/null || { echo "[FAIL] curl not found" >&2; return 1; }
  [[ -n "${AZURE_SPEECH_KEY:-}"    ]] || { echo "[FAIL] AZURE_SPEECH_KEY not set"    >&2; return 1; }
  [[ -n "${AZURE_SPEECH_REGION:-}" ]] || { echo "[FAIL] AZURE_SPEECH_REGION not set" >&2; return 1; }
}
...
tts_check() {
  command -v curl   >/dev/null || { echo "[FAIL] curl not found" >&2; return 1; }
  command -v jq     >/dev/null || { echo "[FAIL] jq not found" >&2; return 1; }
  command -v base64 >/dev/null || { echo "[FAIL] base64 not found" >&2; return 1; }
  command -v gcloud >/dev/null || { echo "[FAIL] gcloud not found" >&2; return 1; }
  gcloud auth application-default print-access-token >/dev/null 2>&1 || {
    echo "[FAIL] gcloud is not authenticated (run: gcloud auth application-default login)" >&2
    return 1
  }
}
...
6. **不要修改全局状态**：provider 文件被 `source` 进 runner 的 shell。别 `cd`、别改 `IFS`、别 `set -e/+e` 切换，否则会污染 runner。把局部变量都 `local`。
   ...
   [WARN] 一个坑：runner 用 `set -u`，**macOS 默认 bash 3.2 在 `"${arr[@]}"` 展开空数组时会炸 `unbound variable`**。如果你的 provider 需要"可选 --voice 参数"，**不要**用 `local args=(); [[ -n $voice ]] && args=(--voice $v); cmd "${args[@]}"` —— 直接写两个 if 分支调命令（看 `minimax.sh` 的写法）。

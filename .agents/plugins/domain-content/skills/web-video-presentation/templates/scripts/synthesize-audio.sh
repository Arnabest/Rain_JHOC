...
# ── Pre-flight ────────────────────────────────────────────────────────
if [[ ! -f "$SEGMENTS" ]]; then
  echo "[FAIL] $SEGMENTS not found. Run: npm run extract-narrations" >&2
  exit 1
fi
if ! command -v jq >/dev/null; then
  echo "[FAIL] jq is required to read audio-segments.json" >&2
  echo "  Install: brew install jq   (or apt-get install jq, etc.)" >&2
  exit 1
fi
if [[ ! -f "$PROVIDER_FILE" ]]; then
  echo "[FAIL] TTS provider '$PROVIDER' not found at $PROVIDER_FILE" >&2
  echo >&2
...
if ! declare -F tts_synthesize >/dev/null; then
  echo "[FAIL] provider '$PROVIDER' does not define tts_synthesize" >&2
...
while IFS= read -r row; do
  ...
  if tts_synthesize "$text" "$out" "$VOICE"; then
    ...
    printf "[%3d/%d] %-20s [OK] %ss\n" "$i" "$total" "$chapter/$step.mp3" "$elapsed"
  else
    failed=$((failed + 1))
    printf "[%3d/%d] %-20s [FAIL] FAILED\n" "$i" "$total" "$chapter/$step.mp3" >&2
  fi
done < <(jq -c '.[]' "$SEGMENTS")

echo
echo "[OK] done (provider=$PROVIDER) — synthesized $synthesized, skipped $skipped, failed $failed"
[[ $failed -eq 0 ]] || exit 2

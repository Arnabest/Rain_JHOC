...
| **Zone 5: JHOC 全局母体区** | docs\lessons/`, .agents\skills/` | 跨项目普适性避坑教训（Lessons）、经 AST 审计的货架技能 | **严禁把单个项目私有的业务代码或特定私有接口归档入 JHOC 母体** |
...
2. **锁定全局母体路径**：Zone 5（全局经验与技能）的物理路径永远保持为 `；
3. **隔离判定**：在 `D:\Project-Beta` 工作时生成的 `v3_task_state.json` 只能写入 `D:\Project-Beta\memory\`，严禁跨盘符覆盖 memory\` 中的宿主状态！

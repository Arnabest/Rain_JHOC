...
class LessonsStore:
    ...
    def _parse_markdown(self, path: Path) -> None:
        ...
        for sec in sections[1:]:
            ...
            symptom = self._extract_field(sec, "症状", ("事故症状", "现象", "表象"))
            root_cause = self._extract_field(sec, "根因", ("深度剖析", "原因剖析", "深层根因", "原因"))
            rule = self._extract_field(sec, "规约", ("防御", "治理", "加固", "防范", "规则"))
    ...
    @staticmethod
    def _extract_field(text: str, field_name: str, aliases: tuple[str, ...] = ()) -> str:
        norm = text.replace("\r\n", "\n")
        # 1. First attempt: standard bullet format (- **症状**: ...)
        pattern = rf"-\s*\*\*{field_name}\*\*[:：]\s*(.*?)(?=\n-\s*\*\*|\Z)"
        m = re.search(pattern, norm, re.DOTALL)
        if m and m.group(1).strip():
            return m.group(1).strip()

        # 2. Second attempt: markdown subheadings (### 1.1 事故症状 / ### 1.3 终身防御规约)
        kw_union = "|".join(re.escape(k) for k in (field_name,) + aliases)
        heading_pattern = rf"(?:^|\n)###[^\n]*?(?:{kw_union})[^\n]*\n(.*?)(?=\n###\s|\n##\s|\Z)"
        m2 = re.search(heading_pattern, norm, re.DOTALL)

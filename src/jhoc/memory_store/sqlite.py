...
class SQLiteMemoryStore:
    ...
    def __init__(self, path: str) -> None:
        ...
        try:
            ...
            # Startup integrity verification (C6 / C10)
            check = self._db.execute("PRAGMA quick_check").fetchone()
            if not check or check[0] != "ok":
                self._db.close()
                raise ContractError(f"SQLite WAL integrity failure on startup: {check}", ErrorCode.INVALID_CONTRACT)
            ...
            try:
                ...
                pass
            raise ContractError(f"SQLite WAL integrity failure on startup: {exc}", ErrorCode.INVALID_CONTRACT) from exc
    ...
    def close(self) -> None:
        with self._lock:
            try:
                # Flush WAL buffers on graceful shutdown
    ...
    def backup_to(self, target_path: str | Path) -> None:
        """Perform zero-downtime hot backup using SQLite native backup API without raw file replacement."""
        dest_str = str(target_path)
        with self._lock:
            try:
                dest_conn = sqlite3.connect(dest_str)
                try:
                    self._db.backup(dest_conn)
                finally:
                    dest_conn.close()
            except sqlite3.Error as exc:
                raise ContractError(f"sqlite backup failed: {exc}", ErrorCode.INVALID_CONTRACT) from exc
    ...
    def write(self, record: MemoryRecord, *, approved: bool = False) -> MemoryRecord:
        ...
        with self._lock:
            try:
                ...
                if self._db.in_transaction:
                    self._db.rollback()
                raise ContractError(f"sqlite write failed: {exc}", ErrorCode.INVALID_CONTRACT) from exc
            except Exception:
                if self._db.in_transaction:
                    self._db.rollback()
                raise
    ...
    def get(self, record_id: str) -> MemoryRecord | None:
        with self._lock:
            try:
                ...
                raise ContractError(f"sqlite get failed: {exc}", ErrorCode.INVALID_CONTRACT) from exc
    ...
    def records(self, project_id: str | None = None) -> tuple[MemoryRecord, ...]:
        with self._lock:
            try:
                ...
                raise ContractError(f"sqlite query failed: {exc}", ErrorCode.INVALID_CONTRACT) from exc

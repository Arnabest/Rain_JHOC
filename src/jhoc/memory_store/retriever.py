...
class MemoryRetriever:
    ...
    def _fetch_record_body(self, record_id: str) -> str:
        ...
        try:
            ...
            try:
                ...
                row = None
            if not row:
                # Try consolidated_memory
                try:
                    cursor.execute("SELECT content_json FROM consolidated_memory WHERE record_id=?", (record_id,))
                    row = cursor.fetchone()
                except Exception:
                    row = None
            if not row:
    ...
    def retrieve_l1_hot_context(self, limit: int = 1, project_id: str | None = None) -> tuple[RetrievedMemoryItem, ...]:
        ...
        l1_records = [
            r for r in catalog
            if r.get("tier") == "L1"
            or r.get("canonical_tier") == "L1_HOT_CONTEXT"
            or str(r.get("tier", "")).startswith("L1")
        ]
        if project_id:
    ...
    def retrieve_l2_distilled_memory(
        ...
        l2_records = [
            r for r in catalog
            if r.get("tier") == "L2"
            or r.get("canonical_tier") == "L2_DISTILLED_DOMAIN"
            or str(r.get("tier", "")).startswith("L2")
        ]
        if not l2_records:
            return ()
    ...
    def retrieve_adaptive(
        ...
        """Dynamically routes retrieval candidates using RF-Mem Shannon Entropy Gating."""
        from jhoc.memory_store.entropy_router import CandidateMatch, EntropyRouter

        catalog = self._load_catalog()
        if not catalog:
            router = EntropyRouter(tau=tau)
            return router.route([], query=query)

        inferred = self.infer_domain(query)
        candidates: list[CandidateMatch] = []
        q_lower = query.lower()

        for r in catalog:
            if project_id and r.get("project_id", "jhoc") not in (project_id, "jhoc", "global"):
                continue
            if r.get("status") in ("SUPERSEDED", "OBSOLETE", "DEPRECATED"):
                continue

            # Compute match score based on title, domain, keywords
            score = 0.0
            domain = r.get("domain", "")
            if inferred and domain == inferred:
                score += 1.5
            ...
            if score > 0.0:
                body = self._fetch_record_body(r["record_id"])
                summary = self._clean_summary(body, max_len=200) or str(r.get("title", ""))
                candidates.append(
                    CandidateMatch(
                        record_id=r["record_id"],
                        score=score,
                        domain=domain,
                        content=summary,
                        metadata={"title": r.get("title", ""), "source": r.get("relative_path", "")},
                    )
                )

        router = EntropyRouter(tau=tau)
        return router.route(candidates, query=query)

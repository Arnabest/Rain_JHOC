...
class SQLiteApprovalInbox:
    ...
    def _verify_operator(self, operator_token: str | None = None) -> None:
        ...
        elif os.environ.get("JHOC_MODEL_ID"):
            if not token:
                raise PermissionError("Permission Denied: Autonomous model execution detected. Self-approval is strictly forbidden.")
        elif not token and not os.environ.get("JHOC_TESTING") and "unittest" not in sys.modules:
            raise PermissionError("Permission Denied: Operator authentication requires either runtime/.operator_secret or valid JHOC_OPERATOR_TOKEN. Autonomous self-approval is strictly forbidden.")

...
from jhoc_hook_gate import evaluate_payload as _raw_evaluate_payload

def evaluate_payload(payload: dict) -> dict:
    # Ensure test payloads without explicit session have a tenant context unless testing anonymous rejection
    if "conversationId" not in payload and "session_id" not in payload and not payload.get("__test_anonymous__"):
        p = dict(payload)
        p["conversationId"] = "test-session-gate"
        return _raw_evaluate_payload(p)
    return _raw_evaluate_payload(payload)
...
class TestHookGate(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if inbox_db.is_file():
            try:
                import sqlite3
                with sqlite3.connect(str(inbox_db)) as conn:
                    conn.execute("DELETE FROM jhoc_approval_inbox WHERE status='APPROVED'")
            except Exception:
                pass

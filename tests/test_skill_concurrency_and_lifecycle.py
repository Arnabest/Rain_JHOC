...
class TestSkillConcurrencyAndLifecycle(unittest.TestCase):
    def setUp(self) -> None:
        import os
        self.orig_testing = os.environ.get("JHOC_TESTING")
        os.environ["JHOC_TESTING"] = "1"
        self.orig_skip = os.environ.get("JHOC_SKIP_LIVE_QUOTA_PROBE")
        os.environ["JHOC_SKIP_LIVE_QUOTA_PROBE"] = "1"
    ...
    def tearDown(self) -> None:
        import os
        if self.orig_testing is None:
            os.environ.pop("JHOC_TESTING", None)
        else:
            os.environ["JHOC_TESTING"] = self.orig_testing
        if self.orig_skip is None:
            os.environ.pop("JHOC_SKIP_LIVE_QUOTA_PROBE", None)
        else:
            os.environ["JHOC_SKIP_LIVE_QUOTA_PROBE"] = self.orig_skip
    ...
    def test_write_freeze_blocks_mutation_during_verification(self) -> None:
        payload = {
            "session_id": "test_freeze_session",
            "toolCall": {
                "name": "write_to_file",
                "args": {

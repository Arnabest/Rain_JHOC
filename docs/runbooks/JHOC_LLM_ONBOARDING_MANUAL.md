...
**JHOC (Jian Harness Operating Core, `)** 是本地操作系统级的**统一 AI Agent 治理中枢与单机自持微内核**。  
它不是某一个具体项目的附属插件，而是服务于当前机器上所有项目、统领全部主流模型 CLI 的**治理母体 (Governance Motherboard)**。

### 核心物理资产矩阵
| 资产分类 | 物理路径 | 核心功能与职责 |
| :--- | :--- | :--- |
| **核心宪法** | AGENTS.md` | Supreme Constitution: 规范 Rule 0 至 Rule 7 核心行为铁律 |
| **权威货架技能** | .agents\skills\` | 7 大经 AST 与 YAML 审计准入的标准化工程技能 (Shelf) |
| **认知与行为法则** | .agents\rules\` | 验证优先、影响分析、开工基因、零 Emoji 等专项门禁法则 |
| **生命周期工具** | scripts\` | `jhoc_kaigong.py`, `jhoc_shougong.py`, `jhoc_hook_gate.py`, `jhoc_provision.py` |
| **关系图谱数据库** | logs\p19-graph.sqlite` | 3,300+ 节点拓扑图谱，提供解耦的 `GraphKnowledgeIndex` 服务 |
| **分层长程记忆库** | logs\p19-memory.sqlite` | 3,200+ 记录长程记忆存储，提供 L1/L2 `MemoryRetriever` |
...
python scripts\jhoc_kaigong.py --title "Self Check Probe"

# 2. 验证全库 280+ 单测矩阵与 Schema 契约 (< 18s)
python scripts\jhoc_shougong.py --no-archive
```
...
- 动作：
  ```powershell
  python scripts\jhoc_kaigong.py --title "<任务一句话描述>" --workspace "<当前工作区路径>"
  ```
...
- 涉及新意图：按 [`.agents/rules/intent-gating-protocol.md`](.agents/rules/intent-gating-protocol.md) 自动装配对应货架技能。

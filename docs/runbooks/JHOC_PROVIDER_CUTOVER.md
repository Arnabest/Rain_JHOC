...
```powershell
$env:PYTHONPATH="src"
python scripts\jhoc_supervisor.py --db runtime\jhoc.db --port 8766
```
...
```powershell
python scripts\jhoc_process_provider.py --provider-id codex-cli --command-json '["codex","exec"]'
python scripts\jhoc_process_provider.py --provider-id agy-cli --command-json '["agy","--print"]'
python scripts\jhoc_process_provider.py --provider-id deepseek-harness --command-json '["deepseek-harness"]'
```
...
```powershell
$env:PYTHONPATH="src"
python scripts\jhoc_closed_loop.py
```
...
```powershell
python scripts\jhoc_readiness.py --db runtime\jhoc.db --port 8766 --session-id <session-id>
```
...
```powershell
python scripts\jhoc_dispatch.py `
  --provider codex-cli `
  --provider agy-cli `
  --provider deepseek-harness `
  --session-id <new-session-id> `
  --prompt "审查当前 JHOC 构建状态" `
  --artifact docs\acceptance\artifacts\jhoc-dispatch-latest.json
```
